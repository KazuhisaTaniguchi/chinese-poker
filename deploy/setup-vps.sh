...
echo ""
echo "  3. Start Holdem project:"
echo "     # .env を作成（DB_PASSWORD を設定）"
echo "     echo 'DB_PASSWORD=your-password-here' > /var/www/holdem/.env"
echo "     # backend/.env.production を作成（.env.production.example を参考に）"
echo "     vi /var/www/holdem/backend/.env.production"
echo "     # 初回ビルド＆起動"
echo "     cd /var/www/holdem && docker compose -f docker-compose.prod.yml --profile build run --rm holdem_frontend_builder"
echo "     cd /var/www/holdem && docker compose -f docker-compose.prod.yml run --rm holdem_backend python manage.py collectstatic --noinput"
echo "     cd /var/www/holdem && docker compose -f docker-compose.prod.yml up -d"
echo ""
echo "  4. Add cron jobs:"
echo "     crontab -e"
